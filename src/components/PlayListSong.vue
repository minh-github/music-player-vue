...
<div>
    <div class="pb-1 flex justify-between items-center mb-2 mt-9">
        ...
        {{ playListInfo.name }}</span>
    </div>
    <div class="overflow-scroll h-[50vh]">
        <div @click="playSong(song.id)" v-for="song in playListSongs" :key="song.id" class="flex py-2 cursor-pointer hover:shadow-md items-center relative" x-data="{openModal: false}">
            <img class="w-10 h-10 object-cover rounded-lg mr-3" alt="thumb" :src="song.thumb" />
